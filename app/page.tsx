import type {ISbResult} from '@storyblok/react';
...
function Features(): ReactElement {
    ...
    return (
        <div className="border-y border-[#d2d2d7]/40">
            <div className="container mx-auto px-6 py-8">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                    {features.map(
                        feature => (
                            <div key={feature.label} className="flex flex-col items-center text-center gap-2">
                                <span className="text-[#1d1d1f]/70">{feature.icon}</span>
                                <span className="text-sm font-medium text-[#1d1d1f]">{feature.label}</span>
                            </div>
                        ),
                    )}
                </div>
...
function Statement(): ReactElement {
    return (
        <div className="bg-[#f5f5f7] py-24 md:py-32">
                ...
                {categories.map(
                    category => (
                        <Link
                            key={category.name}
                            href={category.href}
                            className="group relative rounded-3xl overflow-hidden bg-[#f5f5f7] p-10 md:p-14 flex flex-col justify-end min-h-[240px] hover:bg-[#ededf0] transition-colors"
                        >
                            <h3 className="text-2xl md:text-3xl font-semibold tracking-tight text-[#1d1d1f]">
                                {category.name}
                            </h3>
                            <p className="mt-1 text-[#86868b]">{category.description}</p>
                            <span className="mt-4 inline-flex items-center text-[#06c] text-sm font-medium group-hover:underline">
                                Browse collection
...
export async function fetchData(): Promise<ISbResult> {
