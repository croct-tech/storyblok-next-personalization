...
export function renderMarkdown(text: string): ReactElement {
    return render<ReactElement>(text, {
        ...
        bold: node => <strong key={node.index} className="font-semibold text-primary">{node.children}</strong>,
        italic: node => <em key={node.index}>{node.children}</em>,
        strike: node => <s key={node.index}>{node.children}</s>,
        code: node => <code key={node.index} className="bg-surface-alt px-1.5 py-0.5 rounded text-sm">{node.content}</code>,
        link: node => <a key={node.index} href={node.href} className="text-link hover:underline">{node.children}</a>,
